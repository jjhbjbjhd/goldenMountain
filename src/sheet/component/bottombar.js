  ...
  constructor(addFunc = () => {},
    ...
    this.el = h('div', `${cssPrefix}-bottombar`).children(
      ...
      h('ul', `${cssPrefix}-menu`).children(
        ...
        this.menuEl = h('div','sheet-name-container')
      )
    );
  }

  addItem(name, active, options) {
    this.dataNames.unshift(name);
    const item = h('li', active ? 'active' : '');
    const text = h('span','text-class').child(name);
    const icon = h('span',active ? 'icon-class-active':'icon-class-unActive').child('😧');
    item.child(text)
    item.child(icon);
    ...
    icon.on('click',()=> {
      this.deleteEl = item;
      SheetInstance?.spreadsheet?.deleteSheet();
    })
