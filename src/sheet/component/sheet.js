...
function verticalScrollbarMove(distance) {
  ...
  const clientHight = data.viewHeight();
  const allRowsHight = data.rows.height * data.rows.len;
  const add = (allRowsHight - distance >= clientHight) ? distance : distance + 3 * data.rows.height
  data.scrolly(add, () => {
...
function horizontalScrollbarMove(distance) {
  const { data, table, selector } = this;
  const clientWidth = data.viewWidth();
  const allColsWidth = data.cols.width * data.cols.len;
  const add = (allColsWidth - distance !== clientWidth) ? distance : distance + 2 * data.cols.width;
  data.scrollx(add, () => {
